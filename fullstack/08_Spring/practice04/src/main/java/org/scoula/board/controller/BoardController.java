...
@RestController
@RequestMapping("/api/board") //api는 관례, 이후에 v1과 같은 요소는 버전 정보
@RequiredArgsConstructor
@Log4j2
@Api(tags="게시글 관리") //Swagger 컨트롤러 이름 설정
public class BoardController {
    ...
    @PutMapping("/{no}")
    public ResponseEntity<BoardDTO> update(@ApiParam(value = "게시글 ID", required = true, example = "1")
                                               @PathVariable Long no,
                                           @ApiParam(value = "게시글 객체", required = true) @RequestBody BoardDTO board) {
    ...
    @DeleteMapping("/{no}")
    public ResponseEntity<BoardDTO> delete(@ApiParam(value = "게시글 ID", required = true, example = "1") @PathVariable Long no) {
